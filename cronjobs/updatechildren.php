...
if ( $user->isLoggedIn() )
{
    ...
    else
    {
        ...
        // Fetch array of nodes corresponding to objects in the pending_list to sort them by depth
        ...
        else
        {
            ...
            foreach ( $objectDepthList[1] as $contentobjectId )
            {
                ...
                {
                    // Generate update children data for every pending object in pending_list
                    $paexObjectArray = $pendingList[$contentobjectId]->generateUpdateChildren( $paexObjectArray );
                }
                else
                {
                    eZDebug::writeError( 'Found contentobject_id [' . $contentobjectId . '] not present in the pending_list', 'updatechildren.php' );
                }
            }

            // Reset pending object updatechildren attribute in pending_list objects and add to the array of paex objects to update
            foreach ( $pendingList as $pendingObject )
            {
